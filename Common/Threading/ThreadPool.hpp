...
#define BS_THREAD_POOL_PRIORITY_OUTPUT
	...
	public:
		 ...
		 * @param init_task An initialization function to run in each thread before it starts to execute any submitted tasks. The function must take no arguments and have no return value. It will only be executed exactly once, when the thread is first constructed.
		 */
		thread_pool(const concurrency_t num_threads, const std::function<void()>& init_task) : rethrow_exceptions(false), thread_count(determine_thread_count(num_threads)), threads(std::make_unique<std::thread[]>(determine_thread_count(num_threads)))
		{
		...
		[[nodiscard]] bool is_rethrow_exceptions() const 
		{
			return rethrow_exceptions;
		}

		void set_rethrow_exceptions(bool rethrow_flag) 
		{
			rethrow_exceptions = rethrow_flag;
		}
		 ...
		 */
		template <typename F, typename R = std::invoke_result_t<std::decay_t<F>>, class X = typename std::enable_if<!std::is_void<R>::value>::type>
		[[nodiscard]] std::future<R> submit_task(F&& task BS_THREAD_POOL_PRIORITY_INPUT, std::source_location location = std::source_location::current())
		{
			const std::shared_ptr<std::promise<R>> task_promise = std::make_shared<std::promise<R>>();
			bool rethrow_exception_flag = is_rethrow_exceptions();
			detach_task(
				[task = std::forward<F>(task), task_promise, rethrow_exception_flag, location]
				{
					try
					{
		...
		template <typename F, typename R = std::invoke_result_t<std::decay_t<F>>, class X = typename std::enable_if<std::is_void<R>::value>::type>
		[[nodiscard]] std::future<R> submit_task_void(F&& task BS_THREAD_POOL_PRIORITY_INPUT)
		{
			const std::shared_ptr<std::promise<R>> task_promise = std::make_shared<std::promise<R>>();
			bool rethrow_exception_flag = is_rethrow_exceptions();
			detach_task(
				[task = std::forward<F>(task), task_promise, rethrow_exception_flag]
				{
					try
					{
						task();
					...
					{
						CrashHandler::HandleTermination();
						task_promise->set_exception(std::current_exception());
						if (rethrow_exception_flag) {
							std::rethrow_exception(std::current_exception());
						}
