...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      StackEntry* newEntry = new StackEntry(entry.lineFileName, entry.name, 0);
      entries.push_back(newEntry);
    }
	else {
        StackEntry* newEntry = new StackEntry(entry.lineFileName, entry.name, entry.lineNumber);
        entries.push_back(newEntry);
...
std::ostream& operator<<(std::ostream& out, const StackEntry* entry) {
	out << "[FILE: " << entry->getSourceFile() << "|" << entry->getCalledFunction() << "(#" << entry->getLine() << ")]";
	return out;
}
